...
//			coutd.setVerbose(true);
			...
			for (size_t i = 0; i < lm_tx->lme->tx_offset; i++) {
				mac_layer_me->update(1);
				mac_layer_you->update(1);
				mac_layer_me->execute();
				mac_layer_you->execute();
...
//			coutd.setVerbose(false);
		...
		void testLinkRenewalChannelChangeMultiSlot() {
			...
			lm_me->updateTrafficEstimate(num_outgoing_bits);
			unsigned int required_slots = lm_me->estimateCurrentNumSlots();
			CPPUNIT_ASSERT_EQUAL(expected_num_slots, required_slots);
			// Now do the other tests.
//			coutd.setVerbose(true);
			...
			for (const auto& channel : mac_layer_me->reservation_manager->getP2PFreqChannels()) {
				const ReservationTable *table_me = mac_layer_me->getReservationManager()->getReservationTable(channel),
						*table_you = mac_layer_you->getReservationManager()->getReservationTable(channel);
				for (size_t t = 0; t < lm_me->lme->tx_offset; t++) {
...
//			coutd.setVerbose(true);
			...
			do {
				...
				mac_layer_you->onSlotEnd();
				mac_layer_me->notifyOutgoing(num_outgoing_bits, communication_partner_id);
			} while (lm_tx->current_reservation_table->getReservation(0).isIdle() || lm_tx->current_reservation_table->getReservation(0).isTxCont());
			...
			for (size_t t = 0; t <= lm_tx->lme->getExpiryOffset(); t++) {
				...
				coutd << std::endl;
				// Matching reservations.
				unsigned int burst_length = lm_tx->lme->tx_burst_num_slots;
				if (t % lm_tx->lme->tx_offset == 0) {
					for (size_t i = 0; i < burst_length; i++) {
						coutd << "i=" << i << ": " << lm_tx->current_reservation_table->getReservation(t+i) << std::endl;
						if (i == 0) {
							CPPUNIT_ASSERT_EQUAL(Reservation(communication_partner_id, Reservation::TX), reservation_tx);
							CPPUNIT_ASSERT_EQUAL(Reservation(own_id, Reservation::RX), reservation_rx);
						} else {
							CPPUNIT_ASSERT_EQUAL(Reservation(communication_partner_id, Reservation::TX_CONT), lm_tx->current_reservation_table->getReservation(t+i));
							CPPUNIT_ASSERT_EQUAL(Reservation(own_id, Reservation::RX_CONT), lm_rx->current_reservation_table->getReservation(t+i));
						}
					}
				}
			}
			...
			for (size_t t = 0; t < lm_tx->lme->getExpiryOffset() * 2; t++) {
				for (const FrequencyChannel* channel : lm_tx->reservation_manager->getP2PFreqChannels()) {
					...
					if (*channel == *lm_tx->current_channel) {
						...
						if (!res_rx.isIdle()) {
							CPPUNIT_ASSERT(res_tx == Reservation(communication_partner_id, Reservation::TX) || res_tx == Reservation(communication_partner_id, Reservation::TX_CONT));
							if (res_tx == Reservation(communication_partner_id, Reservation::TX)) {
								CPPUNIT_ASSERT(t % lm_tx->lme->tx_offset == 0);
								num_res_rx++;
							}
						}
...
//			coutd.setVerbose(true);
			...
			num_slots = 0;
			while (lm_rx->link_establishment_status != LinkManager::link_not_established && num_slots++ < max_num_slots) {
					mac_layer_me->update(1);
					mac_layer_you->update(1);
					mac_layer_me->execute();
					mac_layer_you->execute();
			...
			for (auto channel : mac_layer_me->reservation_manager->getP2PFreqChannels()) {
				for (size_t t = lm_tx->lme->tx_burst_num_slots + 0; t < planning_horizon; t++) {

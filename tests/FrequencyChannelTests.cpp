...
class FrequencyChannelTests : public CppUnit::TestFixture {
	...
	public:
		...
		void testEquality() {
			FrequencyChannel* other = new FrequencyChannel(channel->isPointToPointChannel(), channel->getCenterFrequency(), channel->getBandwidth());
			CPPUNIT_ASSERT_EQUAL(true, *channel == *other);
			delete other;
			other = new FrequencyChannel(!channel->isPointToPointChannel(), channel->getCenterFrequency(), channel->getBandwidth());
			CPPUNIT_ASSERT_EQUAL(false, *channel == *other);
			delete other;
			other = new FrequencyChannel(channel->isPointToPointChannel(), channel->getCenterFrequency() + 1, channel->getBandwidth());
			CPPUNIT_ASSERT_EQUAL(false, *channel == *other);
			delete other;
			other = new FrequencyChannel(channel->isPointToPointChannel(), channel->getCenterFrequency(), channel->getBandwidth() - 1);
			CPPUNIT_ASSERT_EQUAL(false, *channel == *other);
			delete other;
		}

	...
	for (size_t i = 0; i < packet->getHeaders().size(); i++) {
		...
		switch (header->frame_type) {
			...
			case L2Header::link_establishment_reply: {
				...
				processLinkReplyMessage((const L2HeaderLinkEstablishmentReply*&) header, (const LinkManager::LinkEstablishmentPayload*&) payload, packet->getOrigin());				
				break;
			}
...
void LinkManager::processLinkRequestMessage(const L2Header*& header, const L2Packet::Payload*& payload, const MacId& origin_id) {
	coutd << *this << "::processLinkRequestMessage" << std::endl;
	throw std::runtime_error("not implemented");
}

void LinkManager::processLinkReplyMessage(const L2HeaderLinkEstablishmentReply*& header, const LinkManager::LinkEstablishmentPayload*& payload, const MacId& origin_id) {

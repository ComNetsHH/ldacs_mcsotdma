...
BCLinkManager::BCLinkManager(const MacId& link_id, ReservationManager* reservation_manager, MCSOTDMA_Mac* mac,
	...
	link_management_entity->setTxTimeout(0);
	// Offset to next broadcast will be dynamically chosen.
	link_management_entity->setTxOffset(0);
}
...
void BCLinkManager::notifyOutgoing(unsigned long num_bits) {
	...
	if (!broadcast_slot_scheduled) {
		link_management_entity->setTxOffset(broadcastSlotSelection());
		assert(current_reservation_table && "BCLinkManager::notifyOutgoing for unset reservation table.");
		current_reservation_table->mark(link_management_entity->getTxOffset(), Reservation(link_id, Reservation::TX));
		coutd << "scheduled broadcast next_broadcast_slot in " << link_management_entity->getTxOffset() << " slots." << std::endl;
...
L2Packet* BCLinkManager::onTransmissionSlot(unsigned int num_slots) {
	...
	if (!is_beacon_slot) {
		...
		if (mac->isThereMoreData(link_id)) {
			...
			coutd << "scheduled next broadcast in " << link_management_entity->getTxOffset() << " slots -> ";
		} else {
...
unsigned int BCLinkManager::broadcastSlotSelection() const {
	...
	std::vector<int32_t> candidate_slots = current_reservation_table->findCandidateSlots(link_management_entity->getMinOffset(), num_candidates, 1, true);
